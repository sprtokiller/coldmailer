...
<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-2xl font-semibold text-gray-800">Outreach Pipelines</h1>
        <p class="text-sm text-gray-400 mt-1">Run the 7-step AI pipeline to find and contact partners.</p>
      </div>
      ...
      >
        ...
        <div v-if="run.steps.length > 0">
          ...
          <p class="text-xs text-gray-400">
            {{ stepCount(run.steps).done }} / {{ stepCount(run.steps).total }} steps complete
          </p>
        </div>
        <p v-else class="text-xs text-gray-400">No steps run yet</p>
      </NuxtLink>
    </div>
  </div>
</template>
