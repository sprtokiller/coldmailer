        ...
        <tbody class="divide-y divide-gray-50">
          ...
          <tr v-else-if="!partners?.length">
            <td colspan="7" class="text-center py-12">
              <p class="text-gray-400 text-sm">{{ search ? 'Žádný partner nenalezen' : 'Zatím žádní oslovení partneři' }}</p>
              <button
                v-if="search && canCreate"
                class="mt-3 text-sm font-medium text-indigo-600 border border-indigo-200 px-4 py-2 rounded-lg hover:bg-indigo-50 transition-colors"
                @click="navigateTo(`/records?tab=PARTNER&create=1`)"
              >
