...
export default defineEventHandler(async (event) => {
  ...
  if (run.authorId !== user.id) {
    throw createError({ statusCode: 403, message: 'Pouze autor může pipeline upravit.' })
  }
  ...
  if (body.visibility !== undefined) {
    if (!['public', 'private'].includes(body.visibility)) {
      throw createError({ statusCode: 400, message: 'visibility musí být "public" nebo "private".' })
    }
