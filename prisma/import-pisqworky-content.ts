/**
 * Import pIšQworky content files from prisma/pisqworky-content/ into the database.
 *
 * Run with: bun prisma/import-pisqworky-content.ts
 *
 * Imports:
 * - DATA01.txt → GlobalRecord (type: COMPETITION)
 * - MARKET.txt → SystemPrompt (stepType: MARKET_SCANNING)
 * - pIšQworky*.txt → ContextPart (selling points / knowledge base)
 */
...
const PISQWORKY_DIR = join(import.meta.dir, 'pisqworky-content')
...
async function main() {
  ...
  // ── 3. Import pIšQworky content files as ContextPart ──────────────────────
...
async function importCompetitions(userId: string) {
  const filePath = join(PISQWORKY_DIR, 'DATA01.txt')
...
async function importMarketPrompt(userId: string) {
  const filePath = join(PISQWORKY_DIR, 'MARKET.txt')
  const content = readFileSync(filePath, 'utf-8').trim()

  const existing = await prisma.systemPrompt.findFirst({
    where: { name: 'pIšQworky Market Scanner', isSystem: true },
  })
  ...
  if (existing) {
    ...
    console.log('  ↻ Updated: pIšQworky Market Scanner\n')
  } else {
    await prisma.systemPrompt.create({
      data: {
        name: 'pIšQworky Market Scanner',
        content,
    ...
    console.log('  ✓ Created: pIšQworky Market Scanner\n')
  }
}

async function importContextParts(userId: string) {
  const contentFiles = [
    'pIšQworky - Mise, cílová skupina a filozofie soutěže.txt',
    'pIšQworky - Struktura soutěže.txt',
    'pIšQworky - Herní mechanika, dovednosti a charakter soutěže.txt',
  ]

  for (const filename of contentFiles) {
    const filePath = join(PISQWORKY_DIR, filename)
    const content = readFileSync(filePath, 'utf-8').trim()
    const name = `pIšQworky: ${filename.replace('pIšQworky - ', '').replace('.txt', '')}`
