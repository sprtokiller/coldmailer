...
<template>
  ...
  <div v-if="stepType !== 'MARKET_SCANNING'" class="px-5 py-3 border-b border-gray-100 flex gap-2 flex-wrap flex-shrink-0">
    <template v-if="!isOutputStep">
      ...
      <button :class="['text-xs px-3 py-1.5 rounded-lg border transition-colors', activeAddPanel === 'import' ? 'border-purple-400 bg-purple-50 text-purple-700' : 'border-gray-200 text-gray-600 hover:border-gray-300 hover:bg-gray-50']" @click="activeAddPanel = 'import'">↑ Importovat</button>
      <button :class="['text-xs px-3 py-1.5 rounded-lg border transition-colors', activeAddPanel === 'db' ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-600 hover:border-gray-300 hover:bg-gray-50']" @click="activeAddPanel = 'db'">🔍 Z databáze</button>
