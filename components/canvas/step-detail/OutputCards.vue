...
<template>
    ...
    <div v-for="(profile, i) in ppProfiles" :key="i" class="p-5">
      ...
      <div v-if="getArr(profile, 'contacts').length > 0" class="mb-2">
        ...
        <div class="space-y-1">
          ...
          <button v-if="getArr(profile, 'contacts').length > 3 && expandedCardIdx !== i" class="text-xs text-gray-400 hover:text-gray-600" @click="toggleCard(i)">+ {{ getArr(profile, 'contacts').length - 3 }} dalších</button>
        </div>
      </div>
  ...
  <div v-else-if="stepType === 'VALUE_ALIGNMENT'" class="flex-1 overflow-y-auto divide-y divide-gray-100">
    ...
    <div v-for="(alignment, i) in vaAlignments" :key="i" class="p-5">
      ...
      <p v-if="getStr(alignment, 'hookHypothesis')" :class="['text-xs text-indigo-700 bg-indigo-50 rounded px-3 py-2 italic leading-relaxed', expandedCardIdx !== i ? 'line-clamp-2' : '']">{{ getStr(alignment, 'hookHypothesis') }}</p>
      <template v-if="expandedCardIdx === i">
