...
function sourceCountLabel(n: number) {
  if (n === 1) return '1 zdroj'
  if (n >= 2 && n <= 4) return `${n} zdroje`
  return `${n} zdrojů`
}
  ...
  >
    <div class="p-4 border-b border-gray-100">
      <div class="flex items-center justify-between mb-1">
        <span class="text-xs font-medium text-gray-400 uppercase tracking-wide">Krok 1</span>
    ...
    <!-- Source cards — each clickable to filter overlay -->
    ...
    <div class="p-4">
      <template v-if="total > 0">
        <div class="text-xs text-gray-400">{{ total }} záznamů</div>
      </template>
      <template v-else>
        <button
